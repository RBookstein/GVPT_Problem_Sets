...
::: {.callout-tip title="HG comments"}
Correct! But there is information that you can include here. This includes the averages and standard deviations of `libpct_m` and `womleg_2015`.
:::
...
When zero percent of the mass public of a state identify as Liberal, you predict that the percentage of that state's legislature that identify as a woman will be `r tidy(wl15reg) |> filter(term == "(Intercept)") |> pull(estimate) |> round(2)` percent on average. 

For every one percentage point increase in the percent of the mass public of a state that identify as Liberal, you predict that the percentage of a state's legislature that identify as a woman will increase by `r tidy(wl15reg) |> filter(term == "libpct_m") |> pull(estimate) |> round(2)` percentage points on average. You needed to include direction in your interpretation. 
:::
...
::: {.callout-tip title="HG comments"}
The $R^2$ value tells us what proportion of the variation in our dependent variable is explained by variation in our independent variables. 
...
As seen from the data above, the predicted value for Colorado is 26.36756 and Georgia is 21.71096. 

::: {.callout-tip title="HG comments"}
Correct!
:::

The residual value is 4.0634, which is found from the equation 21.67878-17.61538.
...
::: {.callout-tip title="HG comments"}
When working with crosstabs, it is better to refer to the proportions within and between groups. This allows you to more easily compare these values. 
:::
